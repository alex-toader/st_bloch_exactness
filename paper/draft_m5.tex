...
\begin{abstract}
\emph{Problem.}
The standard Bloch-periodic extension of discrete exterior calculus (DEC) on unstructured polyhedral meshes breaks the discrete de~Rham exact sequence: $d_1(\mathbf{k})\,d_0(\mathbf{k}) \neq 0$ for generic wave vector~$\mathbf{k}$. This failure produces spectral pollution in the curl--curl eigenvalue problem, compromises the discrete Hodge splitting, and prevents mesh convergence of the acoustic dispersion relation.
%
\emph{Construction.}
...
%
\emph{Results.}
Verification on five periodic polyhedral complexes---including Voronoi foams and random meshes---confirms that exactness eliminates the pollution, restores the Hodge decomposition, and yields numerically second-order convergent ($O(h^2)$) acoustic dispersion with clean band structure across the full Brillouin zone.
\end{abstract}

\begin{keyword}
discrete exterior calculus \sep Bloch boundary conditions \sep Maxwell eigenvalues \sep polyhedral meshes \sep de~Rham exactness \sep spectral pollution
\end{keyword}
...
\end{enumerate}
Items~(1)--(2) are mathematically new: the exactness failure of the standard Bloch extension on unstructured meshes has not, to our knowledge, been previously identified, and the face-boundary recurrence that corrects it is a new construction. Items~(3)--(4) provide the first numerical evidence that this structural error produces spectral pollution, broken Hodge splitting, and convergence failure on realistic foam geometries.
...
\end{equation}
where $\sigma_e$, $\sigma_f$ are primal edge length and face area, and $\sigma^*_e$, $\sigma^*_f$ are their dual counterparts. For the SC cubic benchmark with lattice constant~$a$, all stars are uniform: $\star_1 = a\,I$, $\star_2 = a\,I$.
...
Let $\varphi_e(\mathbf{k}) \in \mathbb{C}^*$ be any phase assigned to edge~$e$, depending only on $\mathbf{k}$ and the lattice shift $\mathbf{n}_e$ (not on $d_0(\mathbf{k})$). Define $\tilde{d}_1(\mathbf{k})[f,e] = d_1[f,e]\cdot\varphi_e(\mathbf{k})$. Then $\tilde{d}_1(\mathbf{k})\,d_0(\mathbf{k}) \neq 0$ for generic~$\mathbf{k}$ on any complex satisfying the hypotheses of Proposition~\ref{prop:break}.
\end{corollary}

\begin{proof}
At vertex $v_i$ shared by consecutive boundary edges $e_{i-1}$, $e_i$ of face~$f$, exactness requires
\begin{equation}\label{eq:ratio}
\varphi_i / \varphi_{i-1} = -d_0(\mathbf{k})[e_{i-1}, v_i]\,/\,d_0(\mathbf{k})[e_i, v_i].
\end{equation}
By hypothesis $\varphi_e$ depends only on $\mathbf{n}_e$. Consider an edge~$e$ that appears in two faces $f_1$ and~$f_2$ with different predecessors $e'$ (in~$f_1$) and $e''$ (in~$f_2$), sharing vertices $v'$ and~$v''$ respectively. The constraint~\eqref{eq:ratio} requires
\[
  \varphi(\mathbf{n}_e) / \varphi(\mathbf{n}_{e'}) = R_1(\mathbf{k}), \qquad
  \varphi(\mathbf{n}_e) / \varphi(\mathbf{n}_{e''}) = R_2(\mathbf{k}),
\]
where $R_1$, $R_2$ are ratios of entries of $d_0(\mathbf{k})$ evaluated at different vertices with different lattice shifts. On an unstructured mesh satisfying the hypotheses of Proposition~\ref{prop:break}, there exist such pairs with $R_1(\mathbf{k}) \cdot \varphi(\mathbf{n}_{e'}) \neq R_2(\mathbf{k}) \cdot \varphi(\mathbf{n}_{e''})$ for generic~$\mathbf{k}$, since $R_1/R_2$ involves exponentials $e^{i\mathbf{k}\cdot(\mathbf{n}_{e'} - \mathbf{n}_{e''})\mathbf{L}}$ that are nonconstant when $\mathbf{n}_{e'} \neq \mathbf{n}_{e''}$.
\end{proof}

The hypothesis---an edge shared by two faces with predecessor edges carrying distinct lattice shifts---holds on every non-tensorial polyhedral complex with boundary-crossing faces, including all Voronoi foams and random meshes tested here.
